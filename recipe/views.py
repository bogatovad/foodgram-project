...
def index(request):
    ...
    if not request.user.is_authenticated:
        return render(request, 'indexNotAuth.html', {'page': page,'tag':name_tag})
    else:
        return render(request, 'indexAuth.html', {'page': page, 'shop_list': shop_list, 'tag':name_tag})
...
def get_tags(data):
    return [Tag.objects.get(pk=int(number_key)) for number_key in dict(data)['tags']]
        ...
        if form.is_valid():
            ...
            for ingredient in ingredients:
                RecipeIngredient.objects.create(
                    recipe=recipe, ingredient=ingredient[0], amount=ingredient[1])
...
def favorite(request):
    ...
    return render(request, 'favorite.html', {'page': page, 'tag':name_tag})
...
def single_recipe(request, id):
    ...
    if request.user.is_authenticated:
        has_bought_recipe = request.user.shop_list.filter(recipe=recipe).exists()
        am_i_follower = request.user.follower.filter(author=recipe.author).exists()
        ...
        'has_bought_recipe':has_bought_recipe, 'am_i_follower':am_i_follower, 'is_it_favorite':is_it_favorite})
    else:
        return render(request, 'singlePage.html', {'recipe': recipe, 'ingredients':ingredients})
...
def author_recipe(request, id):
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'authorRecipe.html', {'author': author, 'page': page, 'tag':name_tag, 'am_i_follower':am_i_follower})
    else:
        return render(request, 'authorRecipe.html', {'author': author, 'page': page, 'tag':name_tag})
...
def save_shop_list(request):
    ...
    for item in shop_list:
        for ingredient in item.recipe.ingredients.all():
            amount = RecipeIngredient.objects.get(recipe=item.recipe, ingredient=ingredient).amount

            if ingredient.title in result_ingredients:
                result_ingredients[(ingredient.title, ingredient.unit)] += amount
            else:
                result_ingredients[(ingredient.title, ingredient.unit)] = amount
    ...
    file.write(content)
    file.close()
    response = RangedFileResponse(request, open(path_file,'rb'), content_type=magic.from_file(path_file, mime=True))
...
def edit_recipe(request, username: str, id_recipe: int):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('index')
    return render(request, 'formRecipe.html', {'form': form, 'edit':True, 'id_recipe':id_recipe})
